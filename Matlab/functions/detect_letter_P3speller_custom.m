function out = detect_letter_P3speller_custom(predicted, word_len, label, letter_ind, markers_seq, params)
    ...
    for n_letter = 1:word_len
        ...
        for j=begin_trial:end_trial
            unknown_speller_code( markers_seq(letter_ind(j))) = ...
                unknown_speller_code(markers_seq(letter_ind(j))) + predicted(j);
        end
